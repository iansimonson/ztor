...
pub const log = @import("util.zig").log;
...
fn cast_from_cptr(comptime T: type, ptr: anytype) T {
...
fn slice_back(data: []const u8) []const u8 {
    return data[data.len - 1 .. data.len];
}

fn slice_front(data: []const u8) []const u8 {
    return data[0..1];
}

pub fn main() !void {
    const f = try std.fs.openFileAbsolute("/Users/ian/Downloads/debian-10.5.0-amd64-netinst.iso.torrent.txt", .{});
    const file = try f.readToEndAlloc(alloc, std.math.maxInt(usize));

    var hash_buffer = [_]u8{undefined} ** 20;
    const info = file[447 .. file.len - 2];
    std.crypto.hash.Sha1.hash(info[0..], hash_buffer[0..], .{});
    std.log.info("First char: {s}", .{slice_front(info)});
    std.log.info("Last char: {s}", .{slice_back(info)});
    std.log.info("Hash: {s}", .{hash_buffer});
    std.log.info("Hash as hex: {x}", .{hash_buffer});

    var parser = zben.Parser.initWithData(alloc, file);
    defer parser.deinit();
    ...
    for (torrent.info.files.items) |item| {
        std.log.info("File:{} - Size: {}", .{ item.name, item.length });
    }
    ...
    const GET_REQUEST = "GET {}?peer_id={}&info_hash={}&port={}&uploaded={}&downloaded={}&compact=1&left={} HTTP/1.1\r\nHost: {}\r\n\r\n";
    const formated_request = try std.fmt.allocPrint(alloc, GET_REQUEST, .{
        announce.path.?,
        escaped_peer_id[0..],
        escaped_hash[0..],
        6881,
    ...
    var as_sized = cast_from_cptr(*http_parser_sized, http_parser);
    as_sized.data = @ptrCast(*c_void, &peer_list);
    defer alloc.destroy(cast_from_cptr(*http_parser_sized, http_parser));
